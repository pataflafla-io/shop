...
export const LoginForm = () => {
  ...
  return (
    <form action={dispatch} className="flex flex-col">
      ...
      <input className="px-5 py-2 bg-gray-200 rounded mb-5" name="email" type="email" />

      <label htmlFor="email">Password</label>
      <input className="px-5 py-2 bg-gray-200 rounded mb-5" name="password" type="password" />

      {state?.success === false && (
        <p className="mb-5 p-2 border-2 border-brand-burnt-peach text-center text-brand-burnt-peach">
          {state.message}
        </p>
      )}
